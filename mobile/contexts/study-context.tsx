...
export function StudyProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveStateToLocalStorage = useCallback(async (stateToSave: StudyData) => {
    if (user) {
      try {
        const key = `estudeaqui_user_data_${user.id}`;
        const dataToSave = {
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center bg-background">
         ...
         <Text className="mt-4 text-lg font-semibold text-foreground">Carregando seus dados...</Text>
      </View>
    );
  }
